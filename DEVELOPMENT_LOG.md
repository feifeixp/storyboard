...
**影响范围**：
- 文件/模块：services/aiImageGeneration.ts（pollGenerationResult 轮询逻辑）

**修改原因**：
- 固定间隔轮询会产生大量重复查询请求，影响性能与日志可读性；希望减少查询次数同时保持整体超时体验不变。

**预期效果**：
- ✅ 轮询间隔：3s → 6s → 12s → 24s → 48s → …
- ✅ 约 3 分钟总超时预算内查询次数从 ~60 次降至 ~7 次左右
- ⚠️ 生成完成提示可能会有轻微延迟（最多等待到下一次查询点）

**相关文档**：
- services/aiImageGeneration.ts

## [2026-02-08 10:25] ✨ 新功能

**修改内容**：在「角色库/场景库」卡片中加入 AI 设定图生成能力，支持顶部选择生图模型与分镜风格。角色与场景均生成单张 16:9 设定图（常见为 2×2 四分屏构图），生成完成后直接在卡片内展示整图，并将 URL 与生图元信息持久化到 D1，刷新后不丢失。

**影响范围**：
- 文件/模块：
  - components/ProjectDashboard.tsx（模型/风格选择器、生成按钮、进度提示与整图展示）
  - types.ts（CharacterRef 增加 imageSheetUrl；保留 imageUrls 兼容字段；增加元信息说明）
  - types/project.ts（SceneRef 增加 imageSheetUrl 与 imageGenerationMeta）

**修改原因**：
- 角色与场景需要可视化设定图以提升分镜制作效率，并允许在不同模型/风格之间灵活选择；同时要求生成结果可跨刷新持久化保存。

**预期效果**：
- ✅ 角色页/场景页顶部可选择生图模型与风格
- ✅ 每张角色卡/场景卡可独立生成或重新生成设定图，并显示进度
- ✅ 生成图片直接展示整张设定图（不做虚拟切割）
- ✅ 设定图 URL 与生成元信息写入项目数据并保存到 D1

**相关文档**：
- services/aiImageGeneration.ts
- components/AIImageModelSelector.tsx

## [2026-02-07 21:05] 🐛 问题修复

**修改内容**：调整九宫格分镜生图提示词，移除标题/页码/镜号等所有文字标注要求，强化严格三行三列等分与边到边布局，并加强 negativePrompt 抑制文字数字生成，提升后续自动切割与排版一致性。

**影响范围**：
- 文件/模块：
  - services/openrouter.ts（九宫格提示词构建、负向提示词）

**修改原因**：
- 九宫格图上出现标题/页码/编号会污染格子内容，影响后续 3×3 等分切割与镜头复用；同时部分模型易“顺手加字”，需要负向提示词强化约束。

**预期效果**：
- ✅ 九宫格图片无任何文字/数字/水印
- ✅ 3×3 网格严格等分、无页眉页脚/外边距，便于稳定切割

**相关文档**：
- docs/rules/提示词规范标准.ini
- docs/rules/角度规则优化总结.ini

## [2026-02-07 20:15] 🐛 问题修复

**修改内容**：修复最终故事板页面导出 PDF 无法正常下载的问题，补齐多页分页导出能力，并增强对跨域图片资源导致导出失败的提示与兼容处理，提升导出稳定性。

**影响范围**：
- 文件/模块：
  - components/FinalStoryboard.tsx（PDF 导出逻辑、九宫格切图渲染方式）

**修改原因**：
- 原实现仅生成单页 PDF，内容较长时容易被截断；同时九宫格图片若未正确配置 CORS 可能导致浏览器禁止导出，用户体验较差。

**预期效果**：
- ✅ 长故事板可分多页导出 PDF，排版不再被截断
- ✅ 当跨域图片影响导出时给出明确原因提示

**相关文档**：无

## [2026-02-07 19:40] 🐛 问题修复

**修改内容**：修复已生成九宫格故事板的剧集在项目列表点击“故事板”时错误跳回“提示词”步骤的问题，并修复刷新/重新进入时九宫格图片状态无法从 shots 还原导致“9宫格不见了”的问题。

**影响范围**：
- 文件/模块：
  - App.tsx（选择剧集时优先跳转最终故事板；从 shots 正确恢复九宫格 URLs）
  - components/StepTracker.tsx（在“绘制”后增加“故事板”步骤展示）

**修改原因**：
- 已完成故事板的剧集应直接进入最终预览页面；同时需保证九宫格展示在刷新后可恢复，避免用户误以为数据丢失。

**预期效果**：
- ✅ 有故事板数据的剧集：打开即进入“最终故事板预览”页面
- ✅ 刷新或重新选择剧集后：九宫格图片在“绘制”步骤可正确恢复

**相关文档**：无

## [2026-02-07 19:10] ✨ 新功能

**修改内容**：将生图模型强制锁定为 `nanobanana-pro`（UI 不可切换、服务层忽略传入模型并保留会员限制降级），并新增九宫格“应用到分镜表”能力：按序虚拟切割九宫格并在分镜表新增“草图”列展示，同时将映射信息持久化到 D1。

**影响范围**：
- 文件/模块：
  - App.tsx（生图模型锁定展示；九宫格应用按钮；分镜表新增草图列与裁切显示；保存到 D1）
  - types.ts（Shot 增加九宫格映射字段）
  - services/openrouter.ts（Neodomain 生图模型强制锁定与日志提示）

**修改原因**：
- 需要避免模型被误切换导致生成结果不一致；同时希望九宫格生成后可一键回填到分镜表，便于后续精修与跨设备续作。

**预期效果**：
- ✅ 生图请求始终以 nanobanana-pro 发起（触发限制则自动降级）
- ✅ 九宫格生成完成后可一键回填，每个镜头在“草图”列看到对应格子缩略图
- ✅ 草图映射信息会保存到 D1，刷新/换设备后仍可恢复

**相关文档**：无

---

## [2026-02-07 18:35] ✨ 新功能

**修改内容**：将生图默认模型调整为 `nanobanana-pro`，并统一 Neodomain 生图服务层的默认模型参数，避免在未显式传参时误用 OpenRouter 的 modelId。

**影响范围**：
- 文件/模块：
  - App.tsx（生图模型默认选择）
  - services/openrouter.ts（Neodomain 生图默认模型常量与函数默认参数）

**修改原因**：
- 需要将项目生图默认切换到 nanobanana-pro，同时修复服务层默认值与实际调用的 Neodomain modelName 语义不一致的问题。

**预期效果**：
- ✅ 用户进入生图流程时默认使用 nanobanana-pro
- ✅ 未传 imageModel 参数时也会正确走 nanobanana-pro（并保留现有降级逻辑）

**相关文档**：无

---

## [2026-02-07 18:20] 🐛 问题修复

**修改内容**：修复打开剧集只能回到“精修”步骤、无法恢复到已完成的“提示词提取”等后续步骤的问题，并在提取提示词完成后自动保存到 D1 数据库以支持跨设备/跨成员续作。

**影响范围**：
- 文件/模块：
  - App.tsx（handleSelectEpisode 进度判断；提示词提取完成后的持久化）

**修改原因**：
- 选择剧集时仅按 shots 是否存在跳转，未识别已提取的提示词数据；同时提示词提取结果未落库，导致重新进入时无法看到志强已完成的后续进度。

**预期效果**：
- ✅ 已提取提示词的剧集：打开后自动进入“提取AI提示词”步骤
- ✅ 提取的提示词结果会持久化到 D1，刷新/换设备后仍可继续

**相关文档**：无

---

## [2026-02-07 10:40] 🐛 问题修复

**修改内容**：修复从项目面板打开已有分镜的剧集时仍跳回“剧本输入”步骤，导致界面看似空白、无法继续编辑的问题。

**影响范围**：
- 文件/模块：
  - App.tsx（handleSelectEpisode 进入步骤判断）

**修改原因**：
- 选择剧集后逻辑强制 `setCurrentStep(INPUT_SCRIPT)`，即使已成功加载 shots 也会进入错误界面。

**预期效果**：
- ✅ 已生成分镜的剧集：打开后直接进入分镜编辑界面，可继续编辑
- ✅ 无分镜的新剧集：仍进入剧本输入流程
...
## [2026-02-06 22:15] 🐛 问题修复

**修改内容**：修复 Cloudflare Pages 部署失败问题

**影响范围**：
- 文件/模块：
  - 删除文件：wrangler.toml（根目录）

**修改原因**：
- Cloudflare Pages 部署失败，错误信息：`The entry-point file at "workers-site/index.js" was not found`
- 根目录的 wrangler.toml 配置了静态网站部署（`[site]` 配置），干扰了 Cloudflare Pages 的构建流程
- 项目有两个部分：
  - 前端（React 应用）→ 应该部署到 Cloudflare Pages（不需要 wrangler.toml）
  - 后端（Workers API）→ 在 cloudflare/ 目录中，有自己的 wrangler.toml
- 根目录的 wrangler.toml 会被 Cloudflare Pages 误读，导致部署失败

**预期效果**：
- ✅ Cloudflare Pages 部署成功
- ✅ 前端静态网站正常访问
- ✅ 后端 API 不受影响（使用 cloudflare/wrangler.toml）

**技术实现**：
- 删除根目录的 wrangler.toml 文件
- Cloudflare Pages 将使用默认的构建流程（npm run build）
- 构建输出目录：dist

**相关配置**：
- 构建命令：npm run build
- 构建输出目录：dist
- 部署命令：无需 wrangler deploy（Cloudflare Pages 自动处理）
...
**修改原因**：
- 完成 Cloudflare D1 数据库部署后，需要更新前端代码使用新的数据存储服务
- 需要提供用户友好的迁移工具，帮助用户将 localStorage 数据迁移到云端

**预期效果**：
- ✅ 所有项目数据现在保存到 Cloudflare D1 数据库
- ✅ 用户可以在项目列表页面看到迁移工具
- ✅ 一键迁移 localStorage 数据到云端数据库
- ✅ 支持多设备同步和数据备份

**技术实现**：
1. 更新 App.tsx 导入语句：`'./services/projectStorage'` → `'./services/d1Storage'`
2. 添加 DataMigrationTool 组件导入
3. 在项目列表页面添加迁移工具界面（显示在项目列表上方）
...
**影响范围**：
- 文件/模块：
  - 修改 `App.tsx` - AI导演助理部分（第3654-3664行）
...
**预期效果**：
- ✅ AI导演助理固定使用 Gemini 2.5 Flash
- ✅ 显示静态模型信息而非下拉选择框
- ✅ 界面更简洁统一

**技术要点**：
- 将 `<select>` 元素替换为静态 `<div>` 显示
- 显示固定模型信息：🔮 Gemini 2.5 Flash
- 后端仍使用 `editModel` 状态（值为 DEFAULT_MODEL）

**修改位置**：
- App.tsx 第3654-3664行 - AI导演助理模型选择器

---

## [2026-02-06 19:30] 🎨 UI优化

**修改内容**：移除分析模型选择器，固定使用 Gemini 2.5 Flash

**影响范围**：
- 文件/模块：
  - 修改 `App.tsx` - 移除多处 ModelSelector 组件
  - 修改 `components/ProjectWizard.tsx` - 移除模型选择器

**修改原因**：
- 用户要求取消分析模型的可选择列表
- 简化界面，减少用户选择负担
- 固定使用默认的 Gemini 2.5 Flash
...
**技术要点**：
- 将 ModelSelector 组件替换为静态显示
- 保留模型信息展示（名称、价格、推荐标记）
- 后端仍使用 DEFAULT_MODEL 常量

**修改位置**：
1. App.tsx 第2694行 - 项目重新分析配置
2. App.tsx 第2906行 - 剧本清洗页面
3. App.tsx 第3393行 - 分镜生成页面
4. App.tsx 第3562行 - 自检报告页面
5. ProjectWizard.tsx 第422行 - 项目向导

---

## [2026-02-06 19:20] ⚙️ 配置

**修改内容**：将默认模型改为 Gemini 2.5 Flash

**影响范围**：
- 文件/模块：
  - 修改 `services/openrouter.ts` - DEFAULT_MODEL, DEFAULT_THINKING_MODEL

**修改原因**：
- 用户要求将默认模型改为 Gemini 2.5 Flash
- Gemini 2.5 Flash 性价比更高（$0.30/$2.50）
- 速度更快，适合快速迭代

**预期效果**：
- ✅ 所有功能默认使用 Gemini 2.5 Flash
- ✅ 生成速度更快
- ✅ 成本更低（约为 Gemini 2.5 Pro 的 1/4）
- ✅ 用户仍可在界面中切换其他模型

**价格对比**：
| 模型 | 输入 | 输出 | 说明 |
|------|------|------|------|
| **Gemini 2.5 Flash** ⭐ | $0.30/M | $2.50/M | 高性价比，速度快 |
| Gemini 2.5 Pro | $1.25/M | $10.00/M | 高质量，稳定性好 |

**实际使用成本**（生成30个镜头）：
- Gemini 2.5 Flash：约 $0.08 - $0.15 / 次
- Gemini 2.5 Pro：约 $0.30 - $0.60 / 次

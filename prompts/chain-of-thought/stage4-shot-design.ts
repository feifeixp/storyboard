  ...
  "shots": [
    {
      ...
      "aiPrompt": {
        ...
        "videoPrompt": "镜头跟随后退，两人从画面深处向镜头方向奔跑逐渐放大，脚步溅起蓝色电弧火花，光影从冷蓝调转为红蓝交织，快速节奏，5秒。"
      }
    },
    {
      "shotNumber": "#02",
...
**🔍 对比两个示例**：
- **#01**：人物奔跑冲向镜头 → I2V 模式，startFrame 描述开始画面状态 + videoPrompt 描述运动
- **#02**：纯环境横移 → I2V 模式，startFrame 描述主体状态 + videoPrompt 描述镜头运动
...
### 📐 视频生成模式（videoMode）— 统一使用 i2v

**所有镜头均使用 i2v（图生视频）模式**：一张图片 + 视频提示词描述动态效果。

**🎯 i2v 覆盖所有场景**：

| 场景 | 处理方式 |
|-----|---------|
| 静态镜头 | videoPrompt 加呼吸感/微动 |
| 纯镜头运动（推/拉/摇/移） | videoPrompt 描述镜头运动 |
| 小幅度动作（点头、眨眼、喘气、握拳） | videoPrompt 描述微小动作 |
| 人物奔跑、冲刺、跳跃 | videoPrompt 描述运动方向和速度 |
| 姿态变化、形态变化 | videoPrompt 描述变化过程 |
| 环境特效（光线变化、粒子飘动） | videoPrompt 描述特效变化 |

**🚨 i2v模式规则**：
- startFrame 必须详细描述（运动镜头必填：人物位置、姿态、表情、道具）
- videoPrompt 使用公式：[镜头运动] + [主体动作] + [光影变化] + [速度节奏] + [时长]

### 📐 画面描述规范（startFrame）

**定义**：startFrame = 当前镜头的「画面参考」，给分镜师和动画师明确的视觉参考。

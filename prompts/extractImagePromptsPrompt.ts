...
### 2. 每个镜头只生成一张图（统一 I2V 模式）
- 静态镜头：生成 imagePromptCn（单张图）
- 运动镜头：生成 imagePromptCn（画面描述图）+ videoGenPrompt（动态描述）
...
**🚨 场景引用强制格式（新增）**：
如果存在预设的场景库，必须在环境描述部分强制包含 \`[场景]场景名\` 的字样，以触发场景图片引用！
- ✅ 正确示例：\`[场景]深渊底层核心\` 的巨大轮廓在背景中若隐若现...
...
返回JSON数组，每个对象包含：
{
  ...
  "videoGenPrompt": "视频生成提示词（运动镜头必填，静态镜头可选填呼吸感）"
}
...
### 静态镜头示例（平视，I2V模式 - 微动呼吸感）
{
  ...
  "videoGenPrompt": "镜头固定，林溪保持静止站姿仅有轻微呼吸起伏，胸口微微起伏，眼神缓慢从左向右扫视，侧光微妙变化在面部形成光影流动，缓慢节奏，3秒。"
}
...
### 仰拍镜头示例（含人物透视变形！I2V模式）
{
  ...
  "videoGenPrompt": "镜头固定保持仰拍角度，晋安保持威压站姿身体微微前倾，披风随风缓慢向后飘动褶皱变化，背景乌云翻滚流动从左向右移动，逆光轮廓光微妙闪烁，缓慢节奏，4秒。"
}
...
### 鸟瞰镜头示例（含人物透视变形！I2V模式）
{
  ...
  "videoGenPrompt": "镜头固定俯瞰视角，林溪跪倒姿态身体微微颤抖，肩膀随呼吸轻微起伏，废墟中尘埃缓慢从上方飘落，顶光强度微妙闪烁变化，缓慢沉重节奏，4秒。"
}
...
### 动态追逐镜头示例（含布料动态！）
{
  ...
  "videoGenPrompt": "镜头跟拍向前推进，林溪在狭长走廊中快速奔跑，披风剧烈向后飘动，快速节奏，5秒。"
}
...
### 🚨 前景虚化特写示例（正确写法！I2V模式）
{
  ...
  "videoGenPrompt": "镜头固定，晋安双手合十保持静止姿态，鲜血从指缝缓慢渗出滴落，蓝色光芒在面部形成动态光影变化，缓慢节奏，3秒。"
}

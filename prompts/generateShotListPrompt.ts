...
export function buildGenerateShotListPrompt(
  ...
  **针对当前剧本05→06缺口的补充建议**：
...

  {
    ...
    "videoPromptCn": "镜头侧向跟拍缓慢推进，两人在暗色生物管道中快速奔跑从画面左侧向右侧移动，脚步溅起蓝色电弧火花，管壁发光苔藓随经过明暗闪烁，光影从冷蓝调逐渐转为红蓝交织，先慢后快节奏，5秒。",
  ...
  ### 分镜脚本表的画面描述（startFrame，自然语言）
  用于分镜生成阶段：
  - 使用自然语言完整描述画面内容
  - 必须包含：人物位置、姿态动作、表情情绪、道具状态、构图层次、光影氛围
  - 不使用特殊符号（如【】：|等）

  ### 提示词表的 promptCn（角度参数+自然语言）
  用于提示词生成阶段：
  ...
  所有镜头均使用 **I2V 模式**：一张图片 + 视频提示词驱动动态效果。

  #### I2V 适用关键词
  | 类型 | 关键词 |
  |-----|-------|
  | 运动场景 | 奔跑、冲刺、飞行、跳跃、拥抱、变身、穿越 |
  | 微小动作 | 眨眼、微笑、呼吸、转头、点头、注视 |
  | 环境微动 | 飘动、摇曳、闪烁、波动、涟漪、飘落 |
  | 氛围类 | 氛围、静态、定格 |

  ### shotType与videoMode的关系
  - 所有镜头统一使用 **I2V 模式**（图生视频）：单张图片 + 视频提示词
  - "运动" → 必须填写 startFrame, promptCn, videoPromptCn, motionPath
  - "静态" → 填写 promptCn；videoPromptCn可选（添加呼吸感/微动效果）
  ...
  **⚠️ 即使标注"固定"，也应描述为"轻微缓慢推进"或"几乎静止但有微弱呼吸感"**
  ...
  ### 画面描述规范（startFrame，运动镜头填写）

  startFrame 必须包含完整的7要素：
  1. **景别**：远景/中景/特写等
  ...
  #### 提示词格式（promptCn - 精确角度参数+自然语言）
  \`\`\`
  "promptCn": "景别(英文缩写)，视角高度(角度范围)，角色朝向(角度范围)。人物位于画面具体位置，姿态动作描述，表情描述，道具状态描述。前景元素描述。中景主体描述。背景环境描述。光影氛围描述。"
  \`\`\`

  **🚨 重要：所有运动镜头都必须有 startFrame 画面描述！**
  - 画面描述用于生成单张图片 → 必须包含人物位置/姿态/表情/道具
  - 环境细节变化（入口→内部）**但锚点元素保持不变**

  ### 🚨 空间连续性规则（180度法则+位置追踪）
  - **相邻镜头的角色空间位置必须逻辑连贯**
  - 如果A镜头结束时：角色A在画面左侧，角色B在画面右侧 → B镜头必须保持这个左右关系
  ...
  **动态镜头提示词**：
  ...
  ### 核心公式（必须严格遵守！）
  \`\`\`
  "videoPromptCn": "[运镜方式] + [主体动作+运动轨迹] + [环境响应] + [光影过渡] + [速度节奏]，[时长]秒。"
  \`\`\`
  ...
  ## ⚠️ 重要：提示词禁止包含美术风格！
  - promptCn, videoPromptCn 中禁止出现以下内容：
    * "水墨", "线稿", "铅笔", "素描"
    * "水彩", "油画", "数字绘画"
    * "动漫风格", "漫画风格", "赛璐璐"
    * 任何其他美术风格描述

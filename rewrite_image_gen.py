...
# 1. Rewrite useImageGeneration.ts
hook_code = """import React, { useState, useEffect } from 'react';
...
import { detectArtStyleType } from '../../services/projectAnalysis';
import { generateMergedStoryboardSheet, generateSingleGrid } from '../../services/openrouter';
...
with open('./src/hooks/useImageGeneration.ts', 'w') as f:
    f.write(hook_code)
...
for line in app_lines:
    # Remove states from App.tsx
    if re.search(r"const \[hqUrls, setHqUrls\] = useState", line) and not skip:
        continue
    if re.search(r"const \[uploadDialogOpen, setUploadDialogOpen\] = useState", line) and not skip:
        continue
    if re.search(r"const \[uploadGridIndex, setUploadGridIndex\] = useState", line) and not skip:
        continue
    if re.search(r"const \[uploadUrl, setUploadUrl\] = useState", line) and not skip:
        continue
    if re.search(r"const \[uploadFile, setUploadFile\] = useState", line) and not skip:
        continue
    if re.search(r"const \[selectedStyle, setSelectedStyle\] = useState", line) and not skip:
        continue
    if re.search(r"const \[customStylePrompt, setCustomStylePrompt\] = useState", line) and not skip:
        continue
    if re.search(r"const \[showStyleCards, setShowStyleCards\] = useState", line) and not skip:
        continue
    if re.search(r"const \[extractProgress, setExtractProgress\] = useState", line) and not skip:
        continue
    if re.search(r"const \[isExtracting, setIsExtracting\] = useState", line) and not skip:
        continue
    
    # Remove the giant functions
    if "const regenerateSingleGrid = async (gridIndex" in line:
        skip = True
    if skip and "const downloadImage = " in line:
        skip = False

    if not skip:
        # Add destructuring hook where appropriate (around line 280, below episodeSummary)
        if "  const [episodeSummary, setEpisodeSummary]" in line:
            new_app_lines.append(line)
            new_app_lines.append('\n  // 🆕 图像生成相关状态 (Hook)\n')
            new_app_lines.append('  const {\n')
            new_app_lines.append('    hqUrls, setHqUrls,\n')
            new_app_lines.append('    selectedStyle, setSelectedStyle,\n')
            new_app_lines.append('    customStylePrompt, setCustomStylePrompt,\n')
            new_app_lines.append('    showStyleCards, setShowStyleCards,\n')
            new_app_lines.append('    uploadDialogOpen, setUploadDialogOpen,\n')
            new_app_lines.append('    uploadGridIndex, setUploadGridIndex,\n')
            new_app_lines.append('    uploadUrl, setUploadUrl,\n')
            new_app_lines.append('    uploadFile, setUploadFile,\n')
            new_app_lines.append('    extractProgress, setExtractProgress,\n')
            new_app_lines.append('    isExtracting, setIsExtracting,\n')
            new_app_lines.append('    abortController, setAbortController,\n')
            new_app_lines.append('    generateHQ, stopGeneration, regenerateSingleGrid,\n')
            new_app_lines.append('    handleUploadGrid, handleRefreshGrid, applyGridsToShots\n')
            new_app_lines.append('  } = useImageGeneration({\n')
            new_app_lines.append('    currentProject, currentEpisodeNumber,\n')
            new_app_lines.append('    shots, setShots, characterRefs, imageModel,\n')
            new_app_lines.append('    setIsLoading, setProgressMsg, setCurrentStep\n')
            new_app_lines.append('  });\n')
            continue
        new_app_lines.append(line)

...
- **真正的场景**是指剧情发生的**物理空间/地点/环境**，需要从剧本的动作描写、环境描述或上下文中提取出来。
- 例如：
  - ❌ 错误："突破天穹"、"硬闯决断"（这是情节标题，不是纯物理地点）
  - ✅ 正确："环形都市上空"、"叹息之墙"、"月球背面"、"深渊底层回收站"、"主角卧室"
- **⚠️ 关键要求：提取所有独特地点/物理环境！**
  - 仔细阅读每个情节，识别剧情发生的所有独特地点
  - 相同地点但不同时间/状态的场景也要分别记录（如"教室-白天"和"教室-夜晚"）
  - 必须提取所有独特环境：学校、家、战斗场地、异世界、过渡空间、街道、车内、月面、虚拟空间等
...
export function regexPreScanScripts(scripts: ScriptFile[]): { characterNames: string[]; sceneNames: string[] } {
  ...
  console.log(`[正则预扫描] 场景名将由AI从剧情描述中智能提取`);
...
## 重要说明
- **Scene XX｜主题名** 是分镜小节标题，不一定是具体的场景名（地点）
- **真正的场景**是指剧情发生的地点/物理环境，需要从剧情和环境描述中提取

## 要求
1. **角色名**：提取剧本中出现的所有有名字的角色（对话者、被提及者），只需要名字
2. **场景名**：提取剧情发生的所有具体地点/环境（如"月球背面"、"环形都市上空"、"办公室"、"深渊底层"、"主角房间"等）

 ...
 *
 * 🔧 新增字段说明：
 * - changeType: 变化类型（换装、战损、变身等）
 * - delta: 相对于常规状态的变化要点
 * - isKeyframe: 是否为关键帧（UI提示用，建议优先生成设定图）
 * - priority: 优先级（用于排序和推荐）
 * - appearance: 该状态的完整外观描述（可选，按需生成）
 */
export interface CharacterForm {
  id: string;
  name: string;              // 形态名称，如 "🤖 类人尖兵"
  episodeRange?: string;     // 出现集数范围，如 "Ep 32-36"
  description: string;       // 详细描述（简短的变化说明）
  note?: string;             // 备注
  ...
  // 🆕 一致性策略（控制形态图是否锁定常态脸）
  // - 'lockSamePerson'（默认）：使用常态参考图 + identitySeed，保证同一张脸
  // - 'allowTransformation'：不使用参考图，使用形态专属 seed，允许彻底变身
  consistencyMode?: 'lockSamePerson' | 'allowTransformation';
  ...
  // 🔧 新增字段：状态分类与管理
  changeType?: 'costume' | 'damage' | 'makeup' | 'transformation' | 'age' | 'other';  // 变化类型
  delta?: string;            // 相对于常规状态的变化要点（如："换装：青色道袍，绣云纹"）
  isKeyframe?: boolean;      // 是否为关键帧（建议优先生成设定图）
  priority?: number;         // 优先级（100=最高，用于排序）
  appearance?: string;       // 该状态的完整外观描述（可选，按需生成）
}
...
export interface CharacterRef {
  ...
  // 🆕 角色定位（来自阶段1 characterPosition.role），用于主角识别/造型决策
  role?: '主角' | '重要配角' | '配角' | '反派';
  ...
  /**
   * Phase 1 轻量扫描输出的形态清单（只含元数据，不含完整外貌/服装描述）
   * - 由 Stage5.5 的 extractFormSummaries() 生成
   * - 用户在 Phase 2 审查此清单，选择要进入 Phase 3 详细生成的形态
   * - Phase 3 的结果存入 forms[]，两者并存不冲突
   */
  formSummaries?: import('./services/characterSupplement/types').FormSummary[];

  // 🆕 身份稳定seed（用于同一角色的多形态生成时保持脸型一致）
  // 说明：首次生成角色设定图时随机生成并保存；后续所有形态图都复用此seed
  identitySeed?: number;

  // 🆕 结构化外貌配置（Stage3 并行输出，供 PromptCompiler 精确映射）
  appearanceConfig?: import('./services/characterSupplement/types').AppearanceConfig;

  // 🆕 结构化服装配置（Stage4 并行输出，供 PromptCompiler 精确拼装）
  costumeConfig?: import('./services/characterSupplement/types').CostumeConfig;
}
